# Script to encode the SuS-LC images using CLIP's image encoders
...
for model_name in req_models:
    ...
    store_features_path = features_path+"/sus_lc_{}_{}_f_m{}.pt".format(args.prompt_shorthand, dataset, disp_name)
    store_targets_path = features_path+"/sus_lc_{}_{}_t_m{}.pt".format(args.prompt_shorthand, dataset, disp_name)
    ...
    dataset_path = './data/sus-lc/{}/{}'.format(args.dataset, args.prompt_shorthand)

    lc_images = torchvision.datasets.ImageFolder(dataset_path, transform=preprocess)
    dataloader = torch.utils.data.DataLoader(lc_images, batch_size=args.val_batch_size, num_workers=8, shuffle=False)        

    # ------------------------------------------saving features------------------------------------------
    print('start saving sus lc image features')
